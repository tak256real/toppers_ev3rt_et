...
//	sequence = sequence->setNextSequence(	new Sequence(new LineTraceAction(80, 45, new PIDControl(80, 0, 3000)),	new EmptyCondition())		);	// 直線終わりまで
	...
	// 階段
	sequence = sequence->setNextSequence(	new Sequence(new LineTraceAction(50, 80, new PIDControl(40, 0, 0), 0.5),	new DistanceCondition(30000))		);	// スタート直後はゆっくり走行
	...
	sequence = sequence->setNextSequence(	new Sequence(new SitWaitAction(75),											new TimeCondition())		);	// 座って待機
